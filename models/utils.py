...
def generate_double_flow_time_t(flow12, flow21, time_stamp):
    # flow1t: frame1 -> frame t
    flowb1t = flow12.unsqueeze(2) * time_stamp.unsqueeze(1).unsqueeze(3).unsqueeze(4)
    # flow2t: frame2 -> frame t
    flowf2t = flow21.unsqueeze(2) * (1.0 - time_stamp).unsqueeze(1).unsqueeze(3).unsqueeze(4)

    flowf1t = - time_stamp.unsqueeze(1).unsqueeze(3).unsqueeze(4) * flow21.unsqueeze(2)

    flowb2t = - (1.0 - time_stamp).unsqueeze(1).unsqueeze(3).unsqueeze(4) * flow12.unsqueeze(2)
...
def generate_double_coormap_t(flowf1t, flowf2t, flowb1t, flowb2t, coorMap):
    B, C, D, H, W = flowf1t.shape
    coorMap = coorMap.unsqueeze(0).unsqueeze(0).repeat(flowf1t.shape[0], flowf1t.shape[2], 1, 1, 1)
    coorMap = torch.cat((coorMap[:, :, :, :, 1:2], coorMap[:, :, :, :, 0:1]), dim=-1)
    # img1 -> imgt
    coorMapf1_t = coorMap.permute(0, 4, 1, 2, 3) + flowf1t
    coorMapf1_t = coorMapf1_t.permute(0, 2, 3, 4, 1).contiguous().view(B*D, H,W,C)
    coorMapb1_t = coorMap.permute(0, 4, 1, 2, 3) + flowb1t
    coorMapb1_t = coorMapb1_t.permute(0, 2, 3, 4, 1).contiguous().view(B*D, H,W,C)
    # img2 -> imgt
    coorMapf2_t = coorMap.permute(0, 4, 1, 2, 3) + flowf2t
    coorMapf2_t = coorMapf2_t.permute(0, 2, 3, 4, 1).contiguous().view(B*D, H,W,C)
    coorMapb2_t = coorMap.permute(0, 4, 1, 2, 3) + flowb2t
    coorMapb2_t = coorMapb2_t.permute(0, 2, 3, 4, 1).contiguous().view(B*D, H,W,C)
...
def generate_double_coormap(flowf1t, flowf2t, flowb1t, flowb2t, coorMap):
    ...
    coorMapf1_t = coorMapf1_t.permute(0, 2, 3, 1)
    coorMapb1_t = coorMap.permute(0, 3, 1, 2) + flowb1t
    coorMapb1_t = coorMapb1_t.permute(0, 2, 3, 1)
    # img2 -> imgt
    coorMapf2_t = coorMap.permute(0, 3, 1, 2) + flowf2t
    coorMapf2_t = coorMapf2_t.permute(0, 2, 3, 1)
    coorMapb2_t = coorMap.permute(0, 3, 1, 2) + flowb2t
    coorMapb2_t = coorMapb2_t.permute(0, 2, 3, 1)
    return coorMapf1_t, coorMapb1_t, coorMapf2_t, coorMapb2_t
...
def generate_coormap(flow1t, flow2t, coorMap):
    ...
    coorMap1_t = coorMap1_t.permute(0, 2, 3, 1)
    # img2 -> imgt
    coorMap2_t = coorMap.permute(0, 3, 1, 2) + flow2t
    coorMap2_t = coorMap2_t.permute(0, 2, 3, 1)
