...
def quantize_per_tensor(t, bit=8, axis=-1):
    if axis == -1:
        ...
        if t_valid.any():
            ...
            # Handle the case when t_valid is empty
            t_min, t_max = 0, 0  # Set default values or appropriate values based on your use case
...
class WarpKeyframe(nn.Module):
    ...
    def forward(self, key_frame, output_flow):
        ...
        next_coords = 2 * next_coords / torch.tensor([[[[W-1, H-1]]]]).to(next_coords) - 1 # normalize [-1,1]
        # print('Key frame: ',key_frame.shape)
        # print('Next coord: ', next_coords.shape)
        image_warp = F.grid_sample(key_frame, next_coords, padding_mode='border', align_corners=True)
        # print('image_warp: ', image_warp.shape)
        # print('===============')
